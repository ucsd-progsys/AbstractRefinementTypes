...
%
We chose $e$ to avoid the clutter of a separate grammar.
In fact, the refinement language is a \emph{separate} one which only 
includes terms that neither diverge nor crash;
one cannot use arbitrary (source level) terms in refinements. 
%
